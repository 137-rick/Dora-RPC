...
/**
 * a client used demomonitor general config
 * 这个是一个使用monitor从发现服务获取到服务器列表的配置的客户端
 * Class GroupClient
...
class GroupClient
{
    ...
    {
        $serverConfig = include($configpath);
    ...
    private function getConfigObjKey($group = "group1")
    {
        $group = trim($group);
        //if there is no config on this group
        if (!isset($this->serverConfig[$group]) || count($this->serverConfig[$group]) == 0) {
            throw new \Exception("there is no config for this group", 100012);
            return;
        }
        $this->groupName = $group;
        // if there is no config can use clean up the block list
        if (isset($this->serverConfigBlock[$group]) && count($this->serverConfig[$group]) <= count($this->serverConfigBlock[$group])) {
            //clean up the block list
            $this->serverConfigBlock[$group] = array();
        }

        //if not specified the ip and port random get one
        do {
            //get one config by random
            $key = array_rand($this->serverConfig[$group]);
            //if not on the block list.
            if (!isset($this->serverConfigBlock[$group][$key])) {
                return $key;
            }
    ...
    /**
     * @param string $group 使用的配置分组名称
     * @param string $ip 若ip和port都指定了，那么使用指定ip
     * @param string $port 若ip和port都指定了，那么使用指定ip
     * @return mixed
     * @throws \Exception
     */
    //get current client
    private function getClientObj($group = "group1", $ip = "", $port = "")
    {
        $connectHost = "";
        $connectPort = "";
        ...
        if ($ip == "" && $port == "") {
            $clientKey = $this->getConfigObjKey($group);
            //$clientKey = $this->serverConfig[$key]["ip"] . "_" . $this->serverConfig[$key]["port"];
            //set the current client key
            $this->currentClientKey = $clientKey;
            $connectHost = $this->serverConfig[$group][$clientKey]["node"]["ip"];
            $connectPort = $this->serverConfig[$group][$clientKey]["node"]["port"];
        } else {
            //using spec
